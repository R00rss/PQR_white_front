...
export default function TicketsFinalizados({
  tickets,
  selectedTicket,
  getData,
  getSelectedItem,
  ...
  async function handleShowPopUp(id: number) {
    //console.log(id);
    const selectedProduct_ = getSelectedItem(id, "Cerrados");
  ...
  const filteredTickets = (): ticket[] => {
    if (search.length == 0) {
      ...
      const filtered = tickets.filter((ticket) =>
        ticket.client.client_name.includes(search)
      );
