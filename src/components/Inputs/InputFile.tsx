...
import icono_eje from "../../assets/icono_eye.png";
import icono_edit from "../../assets/icono_editar.png";
...
import { add_images } from "../../services/files";

import Swal from "sweetalert2";
import withReactContent from "sweetalert2-react-content";
import { add_file } from "../../services/files";

const DEFAULT_STYLES_DRAG_OVER =
  "mt-2 mx-auto w-[100%] h-[150px] p-2 border-2 border-solid border-blue-500 rounded-md flex flex-col items-center justify-center text-center cursor-pointer";
const DEFAULT_STYLES_DROP_ZONE =
  "mt-2 mx-auto w-[100%] h-[150px] p-2 border-2 border-dashed rounded-md border-gray-300 flex flex-col items-center justify-center text-center cursor-pointer";
const DEFAULT_PROMPT_TEXT =
  "Arrastra y suelta tus archivos aquí o haz click para seleccionarlos";
const VALIDS_TYPES = ["image/png", "application/pdf"];

interface props_input_file {
  required: boolean;
  valid_types: string[];
...
export default function InputFile({
  required = false,
  valid_types = VALIDS_TYPES,
  form_data_files,
  set_form_data_files,
}: props_input_file) {
  // const [form_data_files, set_form_data_files] = useState<FormData>(
  //   new FormData()
  // );
  const [prompt_text, set_prompt_text] = useState(DEFAULT_PROMPT_TEXT);
  const [styles_drop_zone, set_styles_drop_zone] = useState(
    DEFAULT_STYLES_DROP_ZONE
  );
  ...
  const total_size = Array.from(form_data_files.getAll("files")).reduce(
    (acc, file) => acc + (file as File).size,
    0
  );
  ...
  function handle_delete_file(file_name: string) {
    ...
    const aux = form_data_files
      .getAll("files")
      .filter((file) => (file as File).name !== file_name);
    const fd = new FormData();
    ...
    <>
      ...
      >
        ...
        {form_data_files.getAll("files").length ? (
          <ul className="rounded-t-lg rounded-b-lg w-full flex flex-col gap-1 justify-center items-start text-xs">
            <li className="grid grid-cols-[0.5fr_3.5fr_1fr_1fr] w-full justify-center items-center bg-slate-800 text-slate-100 rounded-t-lg">
              <div>#</div>
              <div>Nombre</div>
              <div>Peso</div>
              <div></div>
            ...
            {form_data_files.getAll("files").map((file, i) => {
              ...
              const aux_file_name =
                aux_file.name.length > 10
                  ? aux_file.name.slice(0, 20) + "..."
                  : aux_file.name;
              const aux_size = (aux_file.size / 1024).toFixed(2);
              return (
                <li
                  key={uuidv4()}
                  className="odd:bg-slate-300 even:bg-slate-100 grid grid-cols-[0.5fr_3.5fr_1fr_1fr] w-full justify-center items-center"
                >
                  <div>{i + 1}</div>
                  <div onClick={() => handle_preview(aux_file.name)}>
                    {aux_file_name}
                  </div>
                  <div>{aux_size} KB</div>
                  <div>
            ...
            >
              <span>Total: </span>
              <div></div>
              <div>{(total_size / 1024).toFixed(2)} KB</div>
              <div></div>
            </li>
          </ul>
        ) : (
      ...
      <div className="flex justify-center items-center text-sm gap-2 py-2">
        <button
          className=" flex items-center justify-center rounded-2xl lg:w-[150px] p-1 bg-rose-500 text-rose-100 border-rose-500 border-2 hover:bg-rose-100 hover:text-rose-500 duration-500"
          onClick={reset_form_data}
        >
          <p className="hidden lg:block">Eliminar todo</p>
          <img
            src={icono_delete}
            className="block lg:hidden w-[30px] aspect-square object-contain"
            alt="Eliminar Todo"
          />
        </button>
        {/* <button
          className="flex items-center justify-center rounded-2xl w-[180px] h-[40px] p-1 bg-blue-500 text-blue-100 border-blue-500 border-2 hover:bg-blue-100 hover:text-blue-500 duration-500"
        ...
        <button
          className="flex items-center justify-center  rounded-2xl lg:w-[150px] p-1 bg-azul text-slate-100 border-azul border-2 hover:bg-slate-100 hover:text-azul duration-500"
          onClick={() => set_view_all_files(!view_all_files)}
        >
          <p className="hidden lg:block">Ver todo</p>
          <img
            src={icono_eje}
            className="block lg:hidden w-[30px] aspect-square object-contain"
            alt="Ver Todos los items"
          />
        </button>
      </div>
      {view_all_files ? (
        <section className="w-full flex justify-center items-center flex-wrap gap-5">
          {form_data_files.getAll("files").map((file) => {
            return <Thumbnail key={uuidv4()} file={file as File} />;
          })}
        </section>
      ) : (
